...
### Changed
- **fix(fly) 灯笼没飞上天**：终值误算成 `100-h`（bottom 仅 14~38%，滞留下三分之一）→ 改为 `bottom: h%`，实际升到距屏幕底 **62%~86%**（真·天空高度，含标签在上部展开）
- **fly 灯笼提速 + 定时自动飘**：升空时长 普通 7.5~10s→**5~6.8s**、用户盏 10.5~13s→**7~9s**；新增 ambient interval 每 5.2s 检查，同屏 <4 盏自动放一盏随机预存祝福（初始序列期间自然让位、卸载时清理）
